...
st.set_page_config(
    page_title="AI-FundAlloc",
    page_icon=img
)
st.markdown("<h1 style='color: #4287f5 ; text-align:center;'>AI-Driven Mutal Fund Portfolio generator</h1>" , unsafe_allow_html=True)
st.markdown("<h2 style='text-align: center ; color:#EF9917;'>Now get the best portfolio return at your risk level!</h2>" , unsafe_allow_html=True)
...
def find(x):
    ...
    prob.solve()
    with st.spinner(":timer_clock: Wait! Let AI create your portfolio.."):
           time.sleep(2)
    if(prob.status == pulp.LpStatusInfeasible):
        st.warning(f"High value Alert! {x}% of minimum annual return is not possible every year:expressionless: . Try lowering the minimum return value.")
        return
